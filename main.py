...
class TVQAGenerator:
    ...
    async def generate_qa_candidates(
        self, image_caption: str
    ) -> Dict[str, Dict[str, List[dict]]]:
        """Generate QA candidates"""
        qa_candidates = {"system1": {"qa_list": []}, "system2": {"qa_list": []}}
        ...
        system1_tasks = [
            get_model_response(
                ...
                format_qa_generation_prompt(
                    "system1",
                    image_caption,
                    QA_GENERATION["system1"]["candidates_per_model"],
                ),
            )
        ...
        system2_tasks = [
            get_model_response(
                ...
                format_qa_generation_prompt(
                    "system2",
                    image_caption,
                    QA_GENERATION["system2"]["candidates_per_model"],
                ),
            )
    ...
    async def multi_models_evaluation(
    ...
    async def generate_options_and_type(
        self,
        qa_candidates: Dict,
        evaluated_qa: Dict,
        image_caption: str,
        ...
        try:
            parsed_type_domain = json.loads(
                type_domain_response.replace("```json", "")
                .replace("```", "")
                .replace("\n", "")
                .strip()
            )
        ...
        for system_type in ["system1", "system2"]:
            qa_candidates[system_type].update(
                {"options": [""], "img_type": img_type, "domain": domain}
            )

            if evaluated_qa[system_type] is not None:
                selected_indices = (
                    [evaluated_qa[system_type] - 1]
                    if isinstance(evaluated_qa[system_type], int)
                    else [idx - 1 for idx in evaluated_qa[system_type]]
                )
                ...
                for idx in selected_indices:
                    ...
                    prompt = HARD_NEGATIVE_OPTIONS_PROMPT.format(
                        question=qa["question"], correct_answer=qa["answer"]
                    )
                    option_tasks.append(
                        get_model_response(
                            QA_EVALUATION["system2"]["models"][0], prompt, image_path
                        )
                    )
                ...
                for response in option_responses:
                    try:
                        ...
                        qa_candidates[system_type]["options"] = parsed_response.get(
                            "options", [""]
                        )
                    except (json.JSONDecodeError, KeyError) as e:
    ...
    def save_final_qa_dataset(
        self, evaluated_qa, qa_candidates, image_path, image_caption
    ):
        """Collect QA data"""
        ...
        for system_type in ["system1", "system2"]:
            if evaluated_qa[system_type] is not None:
                ...
                selected_qa_list = [
                    qa_candidates[system_type]["qa_list"][idx]
                    for idx in selected_indices
                ]
    ...
    async def generate_qa_from_image_directory(
        self, dir_path, output_dir, batch_size=30
    ):
        ...
        for i in tqdm(
            range(0, len(image_files), batch_size), desc="Processing batches"
        ):
            batch_count += 1
            ...
            for image_file in batch_files:
                ...
                self.logger.info(f"\n{'='*50}")
                self.logger.info(
                    f"Starting Image Processing [{processed_count + 1}/{len(image_files)}]"
                )
                ...
                try:
                    ...
                    self.logger.info(
                        f"[Image {processed_count + 1}] Caption Generation Result:\n{image_caption}"
                    )

                    qa_candidates = await self.generate_qa_candidates(image_caption)
                    self.logger.info(
                        f"[Image {processed_count + 1}] QA Candidates Generation Result:\n{qa_candidates}"
                    )

                    evaluated_qa = await self.multi_models_evaluation(
                    ...
                    self.save_final_qa_dataset(
                        evaluated_qa, qa_candidates, image_path, image_caption
                    )
                    ...
                    try:
                        ...
                        print(
                            f"Error moving file to processed directory: {str(move_error)}"
                        )
                        continue
                    ...
                    print(
                        f"Error occurred while processing image ({image_path}): {str(e)}"
                    )
                    continue

            # Save data after batch processing
            if self.collected_data:
                try:
                    ...
                    print(
                        f"Batch {batch_count} processed and saved: {batch_success}/{len(batch_files)} items successful"
                    )
                    ...
                    # Even if an error occurs, memory is cleaned up
                    self.collected_data.clear()
                    gc.collect()
...
if __name__ == "__main__":
    ...
    # Run asynchronously
    asyncio.run(
        generator.generate_qa_from_image_directory(
            args.input_directory, args.output_directory, args.save_batch
        )
    )
